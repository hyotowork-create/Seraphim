...
/** [2] 슬라이드 목록 — 절별 카드(4줄 초과 시 여러 쪽). 클릭 송출, 드래그 순서변경 */
export function SlideListPanel(): JSX.Element {
  ...
  // 배경 저장/해제 버튼 상태
  const canSaveBg = liveBg.kind === 'image' && liveBg.mediaId != null && song?.bgMediaId !== liveBg.mediaId
  const canClearBg = song?.bgMediaId != null
      ...
      {!song ? (
        ...
        <div className="flex-1 overflow-auto p-2 space-y-1.5 min-h-0">
          {song.verses.map((v, i) => {
            ...
            const curPageInVerse = isActive ? (activePage?.pageInVerse ?? 0) + 1 : 0
            return (
              <div
                key={v.id}
                draggable
                ...
                <div className="flex items-center gap-2 mb-1">
                  ...
                  {isActive && (
                    <span className="text-[10px] text-accent">
                      ● 송출 중{versePages.length > 1 ? ` (${curPageInVerse}/${versePages.length})` : ''}
                    </span>
                  )}
