...
- [x] **M4b** 성경 슬라이드: 책·장·절 + 본문 입력 → 절 단위 자동 슬라이드화, 성경말씀 섹션·검색, 플레이리스트에 성경 항목 지원
- [x] **M5** 전환효과(Fade·지속시간), 배경 영상(mp4/webm 자동반복), 오디오(MP3 재생·볼륨), .zip 백업/복원
- [x] 자막 세로 위치(상단/중앙/하단)·미세 위치·한 화면 줄 수(1~8) 조절
- [x] 출력 비율 16:9 / 4:3 / 화면 채우기 (프로젝터 대응, 레터박스)
- [x] **M6a** 악보 이미지 → 가사 추출 (Google Gemini Vision): 설정에 API 키(안전 저장)·방식, 악보 업로드 → 추출 → 좌(원본)/우(검수 편집) → 곡 저장
- [x] **M7** 온라인 주보 발행: 예배 데이터 폼(찬양 플레이리스트 자동 연동), 모바일 반응형 HTML 자동 생성·실시간 미리보기, HTML 저장, 공개 URL QR 생성
- [ ] M6b 로컬 OCR(Tesseract kor, 오프라인) · PDF 페이지 변환
- [ ] 배포: electron-builder Windows 설치본/Portable 최종 산출(Windows에서)
